  ...
  actions: {
    ...
    async authen(role, login, psw) {
      try {
        ...
        if (res.status === 200) {
          ...
          localStorage.setItem("user", res.data.n_user);
    ...
    async afficherCatPartenaires() {
      try {
        ...
        this.categorie_partenaireaires = rest.data.response;
        // console.log(this.categorie_partenaireaires)
        return this.categorie_partenaireaires;
      } catch (err) {
        console.log(err);
      }
    },
