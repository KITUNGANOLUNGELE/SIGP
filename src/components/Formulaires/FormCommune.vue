  ...
  <q-layout view="hHh lpR fFf">
    ...
    <q-dialog v-model="dialog" :position="position">
      <q-card style="width: 350px">
        ...
        <q-card-section class="text-center q-pa-md">
          <div class="text-weight-bold">Ajouter une Commune</div>
    ...
    <div class="q-pa-md">
      ...
      <q-markup-table>
        <thead>
          <tr>
            <th class="text-left">Id ville</th>
            <th class="text-left">designation ville</th>
          </tr>
        </thead>
        <tbody>
          <tr :key="index" v-for="(commune, index) in communes">
            <td class="text-left">{{ commune.id }}</td>
            <td class="text-left">{{ commune.nom_commune }}</td>
...
import {useStoreCommune}from "src/stores/storeCommune"
...
export default {
  components: {  },
  setup() {
    const store=useStoreCommune();
    const dialog = ref(false);
    const position = ref("top");
    const nom_commune=ref('');
    const id_ville=ref('');
    const communes= computed(()=>{
      return store.communes;
    });
    onMounted(()=>{
    store.afficherCommune();
    })
  ...
  methods:{
    ...
    this.store.ajoutCommune(comm.nom_commune,comm.id_ville)
},
},
};
</script>
