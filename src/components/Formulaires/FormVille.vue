  ...
  <q-layout view="hHh lpR fFf">
    ...
    <q-dialog v-model="dialog" :position="position">
      <q-card style="width: 350px">
        ...
        <q-card-section class="text-center q-pa-md">
          <div class="text-weight-bold">Ajouter une ville</div>
    ...
    <div class="q-pa-md">
      ...
      <q-markup-table>
        ...
        <tbody>
          <tr :key="index" v-for="(ville, index) in villes">
            <td class="text-left">{{ ville.id }}</td>
            <td class="text-left">{{ ville.nom_ville }}</td>
...
import { useStoreVille } from "src/stores/storeVille";
...
export default {
  components: {  },
  setup() {
    const store=useStoreVille();
    const dialog = ref(false);
    const position = ref("top");
    const nom_ville=ref('');
    const code_province=ref('');
  ...
  methods:{
    submitForm(){
      ...
      this.store.ajoutVille(xville.nom_ville,xville.code_province)
    }
  }
};
</script>
